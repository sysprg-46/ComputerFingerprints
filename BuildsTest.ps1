...
<#
---------------------------------------------------------------------------------
Function receives as argument Builds object and return string presenting 
...
function GetProductKey( $obj )
{
   ...
   $Header  = "Microsoft.PowerShell.Core\Registry::HKEY_LOCAL_MACHINE\"
   $path    = $obj.PSParentPath.Replace( "$Header", "" ) + "\" + $obj.PSChildName 
   $ByteArr = $obj.DigitalProductId
   $KeyData = [System.BitConverter]::ToString( $ByteArr[52..66] ).Replace( "-", "," );
   $isWin8  = (  ( ( $ByteArr[ 66 ] -band 0xFF ) %  6 ) -band 1 );
   $ByteArr[ 66 ] = ( $ByteArr[ 66 ] -band 0xF7 ) -bor ( ( $isWin8 -band 2 ) * 4 );  
   $TrTable = "BCDFGHJKMPQRTVWXY2346789";  
   $PK      = "";  
   for ( $i = 24; $i -ge 0; $i-- ) 
   { 
   ...
   # [08..30] ansi [36..47] ansi
   $sub = $ByteArr[08..30]
   $str = " "
   for ( $i = 0; $i -lt $sub.Length; $i++ ){ $str += [char]$sub[ $i ] }
   $str += " "
   $sub = $ByteArr[36..47]
   for ( $i = 0; $i -lt $sub.Length; $i++ ){ $str += [char]$sub[ $i ] }
   ...
   return "" +`
          ...
          $PK.Substring( 10, 5 ) + "-" +`
          $PK.Substring( 15, 5 ) + "-" +` 
          $PK.Substring( 20, 5 ) + " " + $flag + " " + $KeyData + $str  
}
...
function AddProperties( [pscustomobject]$o )
{
   ...
   Add-Member -InputObject $o -type NoteProperty -name ProductKey -value BBBBB-BBBBB-BBBBB-BBBBB-BBBBB 
...
function Show( $arr )
{
   ...
   "ProductKey", "CurrentVersion", "ReleaseId", "UBR", "PSParentPath", "PSChildName" 
   ) 
   ...
   {
      $w = $arr[ $i ].CurrentBuild + " " + $arr[ $i ].ProductKey
      "$w" >> .\BuildsTest.log
      if ( [int]$arr[ $i ].CurrentBuild -ge 10240 ){break} 
   }
   return 0
}
############################# Builds objects collecting starts ######################
###"" > "e:\PS-BuildsHistory-Errors.txt"
...
$Need = @( "BuildBranch","BuildLabEx","CurrentBuild","CurrentVersion","DigitalProductId","EditionID", `
...
for ( $i = 0; $i -lt $temp.Length; $i++ )
{
   ...
   $obj.ProductKey = GetProductKey( $obj )
